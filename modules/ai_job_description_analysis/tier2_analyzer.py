"""
Tier 2 Enhanced Job Analysis
Risk assessment and cultural fit analysis building on Tier 1 results

This module implements the second tier of the 3-tier sequential batch processing system.
Tier 2 analyzes: Stress Level, Red Flags, Implicit Requirements

Processing Schedule: 3:00-4:30 AM (second batch tier)
Target: 1,000-1,500 output tokens per job
...
class Tier2EnhancedAnalyzer:
    """
    Tier 2 Enhanced Analysis - Risk assessment and cultural fit

    Analyzes: Stress Level, Red Flags, Implicit Requirements
    Uses Tier 1 results as context for deeper analysis
    Target: 1,000-1,500 output tokens
    Processing Schedule: 3:00-4:30 AM
    """
    ...
    def __init__(self, model_override: Optional[str] = None):
        ...
        if model_override:
            ...
            logger.info(f"Tier2EnhancedAnalyzer using model override: {model_override}")

    def analyze_job(self, job_data: Dict, tier1_results: Dict) -> Dict:
        """
        Run Tier 2 enhanced analysis on a single job
        ...
        try:
            # Create Tier 2 prompt with Tier 1 context
            prompt = create_tier2_enhanced_prompt([{
                'job_data': job_data,
            ...
            analysis = self._parse_tier2_response(response)

            if not analysis:
                raise ValueError("Failed to parse Tier 2 response")
            ...
            logger.info(
                f"Tier 2 analysis completed for job {job_data['id']}: "
            ...
            logger.error(f"Tier 2 analysis failed for job {job_data.get('id')}: {e}")
            return {
                'success': False,
    ...
    def _parse_tier2_response(self, response: Dict) -> Optional[Dict]:
        """Parse and validate Tier 2 response from Gemini"""
        try:
            ...
            required_fields = ["job_id", "stress_level_analysis", "red_flags", "implicit_requirements"]
            ...
            result["analysis_version"] = "tier2-v1.0"
            result["tier"] = 2
            ...
            logger.error(f"Error parsing Tier 2 response: {e}")
            return None

    def _store_tier2_results(
        self,
        job_id: str,
        analysis: Dict,
        tokens_used: int,
        ...
        """Store Tier 2 results in database"""
        try:
            # Update job_analysis_tiers table
            update_query = """
                UPDATE job_analysis_tiers
                SET tier_2_completed = TRUE,
                    tier_2_timestamp = NOW(),
                    tier_2_tokens_used = %s,
                    tier_2_model = %s,
                    tier_2_response_time_ms = %s
                WHERE job_id = %s
            """
            ...
            logger.info(f"Tier 2 results stored for job {job_id}")

        except Exception as e:
            logger.error(f"Failed to store Tier 2 results for job {job_id}: {e}")
            raise

    def batch_analyze(self, job_ids: List[str], batch_size: int = 50) -> Dict:
        """
        Batch process multiple jobs for Tier 2 analysis
        ...
        """
        logger.info(f"Starting Tier 2 batch analysis for {len(job_ids)} jobs")
        ...
        for i in range(0, len(job_ids), batch_size):
            ...
            for job_id in batch:
                try:
                    # Get job data and Tier 1 results
                    ...
                    if not tier1_results:
                        logger.warning(f"Tier 1 results not found for job {job_id}")
                        results['failed'] += 1
                        continue

                    # Analyze job with Tier 1 context
                    result = self.analyze_job(job_data, tier1_results)
        ...
        logger.info(f"Tier 2 batch analysis completed: {results}")
    ...
    def _get_tier1_results(self, job_id: str) -> Optional[Dict]:
        """
        Fetch Tier 1 results for a job from normalized tables

        Returns the complete Tier 1 analysis as a dict
        """
        try:
            # Query from normalized tables
            ...
            # Parse tier1 data (assuming it's stored as JSON)
    ...
    def get_tier1_completed_jobs(self, limit: int = 100) -> List[str]:
        """
        Get list of job IDs that have Tier 1 complete but need Tier 2
        ...
        try:
            query = """
                ...
                WHERE tier_1_completed = TRUE
                  AND (tier_2_completed IS NULL OR tier_2_completed = FALSE)
                ORDER BY tier_1_timestamp DESC
                LIMIT %s
            """
            ...
            logger.error(f"Failed to get Tier 1 completed jobs: {e}")
            return []
