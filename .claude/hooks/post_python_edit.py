...
def validate_python_syntax(file_path: str) -> dict:
    ...
    try:
        ...
        return {
            'valid': False,
            'message': f"⏱️ Timeout while validating {file_path}"
        }
    except Exception as e:
        return {
            'valid': False,
            'message': f"⚠️ Validation error: {e}"
        }
...
def main():
    ...
    try:
        ...
        full_path = os.path.join(cwd, file_path) if cwd and not file_path.startswith('/') else file_path
        ...
        error_response = {
            "hookSpecificOutput": {
                "message": f"⚠️ Post-edit validation hook error: {e}"
            }
        }
